...
import os
import sys
import fcntl  # Lock de arquivo para evitar múltiplas instâncias
...
# PROTEÇÃO CONTRA MÚLTIPLAS INSTÂNCIAS (LOCK DE ARQUIVO)

# ═══════════════════════════════════════════════════════════

LOCK_FILE = ‘/tmp/bot_elite_pro.lock’
...
print(‘⚠️ Outra instância já está rodando. Encerrando esta.’)
sys.exit(0)
...
# INICIALIZAÇÃO DO BOT COM LIMPEZA DE WEBHOOK/POLLING

# ═══════════════════════════════════════════════════════════

def init_bot():
“”“Inicializa o bot garantindo que não há conflitos anteriores.”””
b = telebot.TeleBot(TOKEN, parse_mode=‘HTML’)

```
# Tenta remover webhook e aguarda para evitar 409
for attempt in range(5):
    try:
        b.remove_webhook()
        print(f'✅ Webhook removido (tentativa {attempt + 1})')
        time.sleep(2)  # Aguarda propagação no servidor do Telegram
        break
    except Exception as e:
        print(f'[AVISO] remove_webhook tentativa {attempt + 1}: {e}')
        time.sleep(3)
...
sinal_str = f”{direction}*{price_usd:.2f}*{adx:.0f}_{rsi:.0f}”
...
```
# Backoff exponencial para reconexão após 409
retry_delay = 5
while True:
    try:
        print('📡 Ativando polling de mensagens...')
        retry_delay = 5  # Reseta delay ao conectar com sucesso
        bot.polling(non_stop=True, timeout=60, long_polling_timeout=60)
    except Exception as e:
        if '409' in str(e):
            print(f'⚠️ Conflito 409 detectado - Aguardando {retry_delay}s antes de reconectar...')
            time.sleep(retry_delay)
            retry_delay = min(retry_delay * 2, 60)  # Backoff: 5→10→20→40→60s (máx)
            try:
                bot.remove_webhook()
                time.sleep(2)
            except Exception:
            ...
            print(f'⚠️ Erro de polling: {str(e)}')
            time.sleep(15)
```
